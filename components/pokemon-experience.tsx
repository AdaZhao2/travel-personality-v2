          ...
          <motion.div ref={orderRef} data-testid="pokemon-order" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="scroll-mt-4 border-t-2 border-white/10 bg-[#fffdf7] p-6 text-[#17142f] sm:p-10">
            ...
            {(stage === "map" || stage === "checkin") && (
              <motion.div ref={mapRef} data-testid="pokemon-map" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="mt-10 rounded-[2rem] border-2 border-[#17142f] bg-[#102542] p-5 text-white shadow-[7px_7px_0_#17142f] sm:p-7">
                <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end"><div><p className="text-xs font-black uppercase tracking-[.2em] text-[#7ff2c6]">Trainer guide · 冒险攻略</p><h2 className="display mt-2 text-balance text-4xl sm:text-5xl">验票完成，伙伴们在前方等你</h2><p className="mt-3 max-w-2xl font-bold leading-relaxed text-white/70">点击地图点位生成完整打卡图片；每一站都会遇见不同的宝可梦伙伴。</p></div><div className="rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-sm font-black text-white/85">推荐路线：真新镇 → 常青森林 → 月见山 → 华蓝海岸 → 石英高原</div></div>
